...
NEGATIVE_TERMS = []
EXCLUDED_DOMAINS = {'smiles.com.br','blog.smiles.com.br','loja.smiles.com.br'}
...
HTML_SOURCES = {'LATAM Pass - Promoções':'https://latampass.latam.com/pt_br/promocoes'}
    ...
    for nome, url in HTML_SOURCES.items():
        ...
        try:
            ...
            for a in soup.select('a[href]'):
                ...
                itens.append({'fonte':nome,'titulo':texto[:160] or '(sem título)','resumo':'','link':href,'publicado_em':'','metodo':'HTML'})
        except Exception as ex:
...
def enviar_email(corpo, assunto=None):
    ...
    if not assunto: assunto = f"✈️ Promoções de Transferência de Milhas (últimos {RECENCIA_DIAS} dias)"
...
def main():
    print(f"🔍 Buscando promoções (inclui Smiles, sem links do domínio Smiles), últimos {RECENCIA_DIAS} dias…")
